...
class TokenExchangeViewController: UIViewController {
    ...
    @IBAction func requestCodeForAPIAuthorization(_: UIButton) {
        UIApplication.user.auth.oneTimeCode(clientID: UIApplication.identityManager.clientConfiguration.clientID) { result in
            switch result {
    ...
    @IBAction func loadWebView(_: Any) {
        ...
        UIApplication.user.auth.webSessionURL(
            clientID: clientID,
