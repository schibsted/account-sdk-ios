...
public extension IdentityManagerProtocol {
    ...
    func login(email: EmailAddress, password: String, scopes: [String] = [], persistUser: Bool,
               useSharedWebCredentials: Bool, completion: @escaping NoValueCallback) {
        return login(username: Identifier(email), password: password, scopes: scopes, persistUser: persistUser,
                     useSharedWebCredentials: useSharedWebCredentials, completion: completion)
    }
